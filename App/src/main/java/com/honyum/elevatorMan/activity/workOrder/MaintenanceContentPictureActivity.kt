...
import kotlinx.android.synthetic.main.activity_maintenance_content_picture.*
...
class MaintenanceContentPictureActivity : BaseActivityWraper() {
    ...
    private var maintItemInfo: MaintenanceContenInfo = MaintenanceContenInfo()

    private var pic_num: Int = 0
    ...
    override fun getLayoutID(): Int {
        return R.layout.activity_maintenance_content_picture
    }
    ...
    override fun initView() {
        ...
        maintItemInfo = bundle.getSerializable("maintItemInfo") as MaintenanceContenInfo

//        var saveDatas = SharedPreferenceUtil.get(maintItemInfo.id+config.userId,maintItemInfo.id)
//        if (saveDatas!=null)
//        {
//            datas = (saveDatas as java.util.ArrayList<Map<String, Any>>)
//        }
        if (maintItemInfo.datas != null) {
            datas = maintItemInfo.datas
        }

        tv_title1.text = maintItemInfo.content

        tv_pic_num.text = "* 需要拍摄" + maintItemInfo.picNum + "张图片"

        pic_num = maintItemInfo.picNum
        ...
        tv_submit.onClick {
            if (datas.size != pic_num) {
                showToast("必须上传" + pic_num + "张图片")
                return@onClick
            }
            val intent = intent
            val bundle = Bundle()
            ...
            if (datas.isNotEmpty())
            {
                SharedPreferenceUtil.save(maintItemInfo.id+config.userId,maintItemInfo.id,datas)
            }
            finish()
        }
